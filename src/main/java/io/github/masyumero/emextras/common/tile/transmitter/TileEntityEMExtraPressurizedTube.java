...
public class TileEntityEMExtraPressurizedTube extends TileEntityEMExtraTransmitter implements IComputerTile, ITileRadioactive {
    ...
    public TileEntityEMExtraPressurizedTube(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        ...
        addCapabilityResolver(gasHandlerManager = new ChemicalHandlerManager.GasHandlerManager(getHolder(EMExtraBoxedPressurizedTube::getGasTanks),
                new DynamicChemicalHandler.DynamicGasHandler(this::getGasTanks, canExtract, canInsert, null)));
        addCapabilityResolver(infusionHandlerManager = new ChemicalHandlerManager.InfusionHandlerManager(getHolder(EMExtraBoxedPressurizedTube::getInfusionTanks),
                new DynamicChemicalHandler.DynamicInfusionHandler(this::getInfusionTanks, canExtract, canInsert, null)));
        addCapabilityResolver(pigmentHandlerManager = new ChemicalHandlerManager.PigmentHandlerManager(getHolder(EMExtraBoxedPressurizedTube::getPigmentTanks),
                new DynamicChemicalHandler.DynamicPigmentHandler(this::getPigmentTanks, canExtract, canInsert, null)));
        addCapabilityResolver(slurryHandlerManager = new ChemicalHandlerManager.SlurryHandlerManager(getHolder(EMExtraBoxedPressurizedTube::getSlurryTanks),
    ...
    @Override
    protected EMExtraBoxedPressurizedTube createTransmitter(IBlockProvider blockProvider) {
        return new EMExtraBoxedPressurizedTube(blockProvider, this);
    }

    @Override
    public EMExtraBoxedPressurizedTube getTransmitter() {
        return (EMExtraBoxedPressurizedTube) super.getTransmitter();
    }
    ...
    private <CHEMICAL extends Chemical<CHEMICAL>, STACK extends ChemicalStack<CHEMICAL>, TANK extends IChemicalTank<CHEMICAL, STACK>> IChemicalTankHolder<CHEMICAL, STACK, TANK> getHolder(BiFunction<EMExtraBoxedPressurizedTube, Direction, List<TANK>> tankFunction) {
        return direction -> {
            EMExtraBoxedPressurizedTube tube = getTransmitter();
