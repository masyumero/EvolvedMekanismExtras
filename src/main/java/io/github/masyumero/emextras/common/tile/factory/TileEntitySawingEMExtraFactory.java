...
public class TileEntitySawingEMExtraFactory extends TileEntityEMExtraFactory<SawmillRecipe> implements ISingleRecipeLookupHandler.ItemRecipeLookupHandler<SawmillRecipe> {
    ...
    protected IOutputHandler<@NotNull SawmillRecipe.ChanceOutput>[] outputHandlers;

    public TileEntitySawingEMExtraFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            EMExtraFactoryOutputInventorySlot secondaryOutputSlot = EMExtraFactoryOutputInventorySlot.at(this, updateSortingListener, xPos, 77);
            //Note: As we are an item factory that has comparator's based on items we can just use the monitor as a listener directly
            EMExtraFactoryInputInventorySlot inputSlot = EMExtraFactoryInputInventorySlot.create(this, i, outputSlot, secondaryOutputSlot, recipeCacheLookupMonitors[i], xPos, 13);
            int index = i;
