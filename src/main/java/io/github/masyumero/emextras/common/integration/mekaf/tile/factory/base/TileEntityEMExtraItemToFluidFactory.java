...
public abstract class TileEntityEMExtraItemToFluidFactory<RECIPE extends MekanismRecipe> extends TileEntityEMExtraAdvancedFactoryBase<RECIPE> {

    private static final int MAX_FLUID = 10_000;

    protected ItemToFluidProcessInfo[] processInfoSlots;
    ...
    protected TileEntityEMExtraItemToFluidFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, errorTypes, globalErrorTypes);
        inputItemSlots = new ArrayList<>();
        outputFluidTanks = new ArrayList<>();

        processInfoSlots = new ItemToFluidProcessInfo[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            processInfoSlots[i] = new ItemToFluidProcessInfo(i, inputSlot[i], outputTank[i]);
        }

        for (ItemToFluidProcessInfo info : processInfoSlots) {
    ...
    protected void sortInventoryOrTank() {
        Map<HashedItem, ItemToFluidRecipeProcessInfo> processes = new HashMap<>();
        List<ItemToFluidProcessInfo> emptyProcesses = new ArrayList<>();
        for (ItemToFluidProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputSlot.isEmpty()) {
                ...
                ItemToFluidRecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(item, i -> new ItemToFluidRecipeProcessInfo());
        ...
        for (Map.Entry<HashedItem, ItemToFluidRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToFluidRecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    ItemToFluidProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, ItemToFluidRecipeProcessInfo> processes, List<ItemToFluidProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, ItemToFluidRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToFluidRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<ItemToFluidProcessInfo> toRemove = new ArrayList<>();
            for (ItemToFluidProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, ItemToFluidRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, ItemToFluidRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToFluidRecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                ItemToFluidProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record ItemToFluidProcessInfo(int process, @NotNull EMExtraAdvancedFactoryInputInventorySlot inputSlot,
                                         @NotNull IExtendedFluidTank outputTank) {}

    public static class ItemToFluidRecipeProcessInfo {

        private final List<ItemToFluidProcessInfo> processes = new ArrayList<>();
        @Nullable
