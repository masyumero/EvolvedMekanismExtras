...
public abstract class TileEntityEMExtraItemToGasFactory<RECIPE extends MekanismRecipe> extends TileEntityEMExtraAdvancedFactoryBase<RECIPE> {

    private static final long MAX_CHEMICAL = 10_000;

    protected ItemToGasProcessInfo[] processInfoSlots;
    ...
    protected TileEntityEMExtraItemToGasFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, errorTypes, globalErrorTypes);
        inputItemSlots = new ArrayList<>();
        outputGasTanks = new ArrayList<>();

        processInfoSlots = new ItemToGasProcessInfo[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            processInfoSlots[i] = new ItemToGasProcessInfo(i, inputSlot[i], outputTank[i]);
        }

        for (ItemToGasProcessInfo info : processInfoSlots) {
    ...
    protected void sortInventoryOrTank() {
        Map<HashedItem, ItemToGasRecipeProcessInfo> processes = new HashMap<>();
        List<ItemToGasProcessInfo> emptyProcesses = new ArrayList<>();
        for (ItemToGasProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputSlot.isEmpty()) {
                ...
                ItemToGasRecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(item, i -> new ItemToGasRecipeProcessInfo());
        ...
        for (Map.Entry<HashedItem, ItemToGasRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToGasRecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    ItemToGasProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, ItemToGasRecipeProcessInfo> processes, List<ItemToGasProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, ItemToGasRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToGasRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<ItemToGasProcessInfo> toRemove = new ArrayList<>();
            for (ItemToGasProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, ItemToGasRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, ItemToGasRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToGasRecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                ItemToGasProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record ItemToGasProcessInfo(int process, @NotNull EMExtraAdvancedFactoryInputInventorySlot inputSlot,
                                       @NotNull IGasTank outputTank) {}

    public static class ItemToGasRecipeProcessInfo {

        private final List<ItemToGasProcessInfo> processes = new ArrayList<>();
        @Nullable
