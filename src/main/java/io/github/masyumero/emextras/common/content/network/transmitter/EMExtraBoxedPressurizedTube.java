...
public class EMExtraBoxedPressurizedTube extends BoxedPressurizedTube implements IEMExtraUpgradeableTransmitter<PressurizedTubeUpgradeData> {

    public EMExtraBoxedPressurizedTube(IBlockProvider blockProvider, TileEntityTransmitter tile) {
    ...
    private long getAvailablePull(ChemicalType chemicalType) {
        if (hasTransmitterNetwork()) {
            return Math.min(EMExtraTTier.getTubePullAmount(this.tier), getTransmitterNetwork().chemicalTank.getTankForType(chemicalType).getNeeded());
        }
        return Math.min(EMExtraTTier.getTubePullAmount(this.tier), chemicalTank.getTankForType(chemicalType).getNeeded());
    }

    @Override
    public long getCapacity() {
        return EMExtraTTier.getTubeCapacity(this.tier);
    }
