...
public class TileEntityEMExtraPlantingFactory extends TileEntityEMExtraMoreMachineFactory<PlantingRecipe> implements IBoundingBlock, IHasDumpButton, ConstantUsageRecipeLookupHandler,
    ...
    public TileEntityEMExtraPlantingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            EMExtraMoreMachineFactoryOutputInventorySlot secondaryOutputSlot = EMExtraMoreMachineFactoryOutputInventorySlot.at(this, updateSortingListener, getXPos(i), 77);
            // Note: As we are an item factory that has comparator's based on items we can just use the monitor as a
            // listener directly
            EMExtraMoreMachineFactoryInputInventorySlot inputSlot = EMExtraMoreMachineFactoryInputInventorySlot.create(this, i, outputSlot, secondaryOutputSlot, recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    @Nullable
    @Override
    public GasInventorySlot getEMExtraSlot() {
        return gasSlot;
    }
