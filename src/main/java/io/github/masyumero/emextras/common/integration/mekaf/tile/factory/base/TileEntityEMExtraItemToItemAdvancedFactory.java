...
public abstract class TileEntityEMExtraItemToItemAdvancedFactory<RECIPE extends MekanismRecipe> extends TileEntityEMExtraAdvancedFactoryBase<RECIPE> {
    ...
    protected ItemToItemProcessInfo[] processInfoSlots;

    protected final List<IInventorySlot> inputSlots;
    protected final List<IInventorySlot> outputSlots;

    protected TileEntityEMExtraItemToItemAdvancedFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, errorTypes, globalErrorTypes);
        inputSlots = new ArrayList<>();
        outputSlots = new ArrayList<>();

        for (ItemToItemProcessInfo info : processInfoSlots) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        processInfoSlots = new ItemToItemProcessInfo[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            EMExtraAdvancedFactoryOutputInventorySlot outputSlot = EMExtraAdvancedFactoryOutputInventorySlot.at(this, updateSortingListener, getXPos(i), 57);
    ...
    protected void sortInventoryOrTank() {
        Map<HashedItem, ItemToItemRecipeProcessInfo> processes = new HashMap<>();
        List<ItemToItemProcessInfo> emptyProcesses = new ArrayList<>();
        for (ItemToItemProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputSlot.isEmpty()) {
                ...
                ItemToItemRecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(item, i -> new ItemToItemRecipeProcessInfo());
        ...
        for (Map.Entry<HashedItem, ItemToItemRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToItemRecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    ItemToItemProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, ItemToItemRecipeProcessInfo> processes, List<ItemToItemProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, ItemToItemRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToItemRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<ItemToItemProcessInfo> toRemove = new ArrayList<>();
            for (ItemToItemProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, ItemToItemRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, ItemToItemRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToItemRecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                ItemToItemProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record ItemToItemProcessInfo(int process, @NotNull EMExtraAdvancedFactoryInputInventorySlot inputSlot,
                                        @NotNull IInventorySlot outputSlot) {}

    private static class ItemToItemRecipeProcessInfo {

        private final List<ItemToItemProcessInfo> processes = new ArrayList<>();
        @Nullable
