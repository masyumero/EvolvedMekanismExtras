...
public class TileEntityEMExtraLiquifyingFactory extends TileEntityEMExtraAdvancedBase<BasicItemStackToFluidOptionalItemRecipe> implements ItemRecipeLookupHandler<BasicItemStackToFluidOptionalItemRecipe> {
    ...
    public TileEntityEMExtraLiquifyingFactory(Holder<Block> blockProvider, BlockPos pos, BlockState state) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            EMExtraAdvancedFactoryOutputInventorySlot outputSlot = EMExtraAdvancedFactoryOutputInventorySlot.at(this, updateSortingAndUnpause, getXPos(i), 57);
            // Note: As we are an item factory that has comparator's based on items we can just use the monitor as a
            // listener directly
            EMExtraAdvancedFactoryInputInventorySlot inputSlot = EMExtraAdvancedFactoryInputInventorySlot.create(this, i, outputSlot, fluidTank, recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    public record NLProcessInfo(int process, @NotNull EMExtraAdvancedFactoryInputInventorySlot inputSlot,
    ...
    protected static class NLRecipeProcessInfo {
        ...
        @Nullable
        private ToIntBiFunction<NLRecipeProcessInfo, TileEntityEMExtraLiquifyingFactory> lazyMinPerSlot;
        private Object item;
        private BasicItemStackToFluidOptionalItemRecipe recipe;
        private int minPerSlot = 1;
        private int totalCount;

        public int getMinPerSlot(TileEntityEMExtraLiquifyingFactory factory) {
