...
public class EMExtraBinInventorySlot extends BasicInventorySlot {
    private static final Predicate<@NotNull ItemStack> validator = stack -> !(stack.getItem() instanceof EMExtraItemBlockBin || stack.getItem() instanceof ItemBlockBin);

    public static EMExtraBinInventorySlot create(@Nullable IContentsListener listener, EMExtraBTier tier) {
        Objects.requireNonNull(tier, "Bin tier cannot be null");
        return new EMExtraBinInventorySlot(listener, tier);
    }

    private final boolean isCreative;
    private ItemStack lockStack = ItemStack.EMPTY;

    private EMExtraBinInventorySlot(@Nullable IContentsListener listener, EMExtraBTier tier) {
