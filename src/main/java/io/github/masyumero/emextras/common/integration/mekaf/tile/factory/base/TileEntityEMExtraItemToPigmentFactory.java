...
public abstract class TileEntityEMExtraItemToPigmentFactory<RECIPE extends MekanismRecipe> extends TileEntityEMExtraAdvancedFactoryBase<RECIPE> {

    private static final long MAX_CHEMICAL = 10_000;

    protected ItemToPigmentProcessInfo[] processInfoSlots;
    EMExtraAdvancedFactoryInputInventorySlot[] inputSlot;
    ...
    protected TileEntityEMExtraItemToPigmentFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<RecipeError> errorTypes, Set<RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, errorTypes, globalErrorTypes);
        inputItemSlots = new ArrayList<>();
        outputPigmentTanks = new ArrayList<>();

        processInfoSlots = new ItemToPigmentProcessInfo[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            processInfoSlots[i] = new ItemToPigmentProcessInfo(i, inputSlot[i], outputTank[i]);
        }

        for (ItemToPigmentProcessInfo info : processInfoSlots) {
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        inputSlot = new EMExtraAdvancedFactoryInputInventorySlot[tier.processes];
        itemInputHandlers = new IInputHandler[tier.processes];
        for (int i = 0; i < tier.processes; i++) {
            inputSlot[i] = EMExtraAdvancedFactoryInputInventorySlot.create(this, i, outputTank[i], recipeCacheLookupMonitors[i], getXPos(i), 13);
            int index = i;
    ...
    protected void sortInventoryOrTank() {
        Map<HashedItem, ItemToPigmentRecipeProcessInfo> processes = new HashMap<>();
        List<ItemToPigmentProcessInfo> emptyProcesses = new ArrayList<>();
        for (ItemToPigmentProcessInfo processInfo : processInfoSlots) {
            ...
            if (inputSlot.isEmpty()) {
                ...
                ItemToPigmentRecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(item, i -> new ItemToPigmentRecipeProcessInfo());
        ...
        for (Map.Entry<HashedItem, ItemToPigmentRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToPigmentRecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    ItemToPigmentProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, ItemToPigmentRecipeProcessInfo> processes, List<ItemToPigmentProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, ItemToPigmentRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToPigmentRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<ItemToPigmentProcessInfo> toRemove = new ArrayList<>();
            for (ItemToPigmentProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, ItemToPigmentRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, ItemToPigmentRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToPigmentRecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                ItemToPigmentProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record ItemToPigmentProcessInfo(int process, @NotNull EMExtraAdvancedFactoryInputInventorySlot inputSlot,
                                           @NotNull IPigmentTank outputTank) {}

    public static class ItemToPigmentRecipeProcessInfo {

        private final List<ItemToPigmentProcessInfo> processes = new ArrayList<>();
        @Nullable
