...
public class RenderEMExtraMechanicalPipe extends RenderTransmitterBase<TileEntityEMExtraMechanicalPipe> {
    ...
    public RenderEMExtraMechanicalPipe(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    protected void render(TileEntityEMExtraMechanicalPipe tile, float partialTick, PoseStack matrix, MultiBufferSource renderer, int light, int overlayLight,
    ...
    @Override
    protected boolean shouldRenderTransmitter(TileEntityEMExtraMechanicalPipe tile, Vec3 camera) {
        ...
        return cachedLiquids.computeIfAbsent(sideOrdinal, s -> new HashMap<>())
                ...
                .computeIfAbsent(stage, s -> {
                    ...
                    model.xBounds(min, max)
                            .zBounds(min, max);
                    return side == Direction.DOWN ? model.yBounds(0, 0.25F + offset) : model.yBounds(0.25F + offset + stageRatio, 1);// Up
                });
    }
